...
1. Read the branch that matches the goal.
2. If the task needs assets or sources, read the media-sources branch first.
3. If the task uses patching, read the live-session-patching branch first.
4. If the task needs export or readiness checks, read the validation branch.
5. If the task needs catalogs or templates, read the catalogs branch.
6. If the task is an active session edit, open the live-session core page before the deeper branch.
