...
function isRecommendedTransition(item) {
  return Boolean(item?.autoSelectable) || normalizeText(item?.status) === "verified";
}
...
async function listRecommendedTransitions(client, options = {}) {
  ...
  if (tags.length > 0) {
    attempts.push({ ...tagFilter, status: "verified" });
    attempts.push(tagFilter);
    attempts.push({ ...tagFilter, autoSelectable: true });
  }
  attempts.push({ status: "verified", autoSelectable: true });
  attempts.push({ autoSelectable: true });
  attempts.push({ status: "verified" });
