...
2. Get the capability directory before choosing taskType, templates, limits, or HLS options. Use categories only when you need a specific branch.
3. For exact transition choices, call clipnode_media_list_transitions with tag/group/status filters instead of scanning the full capability payload.
4. For exact sticker/text-sticker animation choices, call clipnode_media_get_sticker_capabilities and clipnode_media_list_sticker_animations instead of scanning the full capability payload.
...
6. Use the mode map to avoid guessing:
...
const tools = [
  ...
  },
  {
    ...
    description: "Use near the start of every media task. Returns the main capability directory by default. Pass categories[] when you want one or more category files merged into the response, such as fileCapabilities, sourceCapabilities, workflowCapabilities, plannedWorkflowCapabilities, assetCapabilities, templateCapabilities, hlsCapabilities, videoCompressCapabilities, gifEditCapabilities, videoToGifCapabilities, imageEditCapabilities, imageComposeCapabilities, or transitionCatalog.",
    inputSchema: {
      type: "object",
      properties: {
        categories: {
          type: "array",
          items: { type: "string" },
          description: "Optional category ids to expand and merge into the response. Examples: fileCapabilities, sourceCapabilities, workflowCapabilities, plannedWorkflowCapabilities, assetCapabilities, templateCapabilities, hlsCapabilities, videoCompressCapabilities, gifEditCapabilities, videoToGifCapabilities, imageEditCapabilities, imageComposeCapabilities, transitionCatalog."
        }
      }
    }
  },
